...
def MC_params(cel, disp, dx, dt, n_reach):
    ...
    for i in range(n_reach):
        ...
        c1_mc[i] = (dt - 2.0 * k_MC * x_MC) / (2 * k_MC * (1 - x_MC) + dt)
        c2_mc[i] = (dt + 2.0 * k_MC * x_MC) / (2 * k_MC * (1 - x_MC) + dt)
...
@njit
def MC(
    current_reach, Qupstream, nstep, Qsubreach_in, Qsubreach_out, c1_mc, c2_mc, c3_mc
):
