...
from ictop_utils import init_basin_vars, sort_df_by_idba_seq
from ictop_utils import read_json_seq, init_elev_ener_vars, init_meteo
from module_snow import snow
...
WE_THRESHOLD = 20.  # for SCA
FLOAT_FORMAT_SD = '%.4f'
# first hour is initial condition

if __name__ == '__main__':
    
    start_time = pd.to_datetime(START_TIME, format = '%Y-%m-%d %H:%M') + pd.Timedelta(1, 'h')
    ...
    df_EI = pd.read_csv(
        os.path.join(INPUT_FOLDER, EEB_FOLDER, 'EI.txt'),
        skipinitialspace=True
    )
    ...
    # init elev and energy bands arrays, they are already ordered by model sequence
    n_fasce, n_bande, fasce_area, bande_area, fasce_elev_avg, bande_area_glac, EI = (
    ...
    #     os.path.join(INPUT_FOLDER, INITCOND_FOLDER, 'state_var_SNOW_freezed.txt'),
    #     skipinitialspace=True
    ...
    #     os.path.join(INPUT_FOLDER, INITCOND_FOLDER, 'state_var_SNOW_melt.txt'),
    #     skipinitialspace=True
    # )['value'].values.reshape(n_basin, n_fasce, n_bande)
    WE = pd.read_csv(
    ...
    WE, rhosnow, WE_basin, snowfall_basin, rainfall_basin, baseflow = snow(
        n_fasce=n_fasce, n_bande=n_bande, basin_id=basin_id,
        day_array=day_array, year_array=year_array, month_array=month_array, hour_array=hour_array,
        n_basin=n_basin, n_hours=n_hours, fasce_area=fasce_area, bande_area=bande_area, 
        basin_area=basin_area, fasce_elev_avg=fasce_elev_avg, basin_elev=basin_elev,
        t_intercept=t_intercept, t_slope=t_slope, id_lapse_rate=id_lapse_rate, n_lapse=len(id_lapse_rate),
        basin_lapse=basin_lapse, t_idlapse=t_idlapse, prec=prec,
        PCF=PCF, precgrad=precgrad, 
        tsnow=tsnow, tmelt=tmelt,
        snowfall=np.full((n_basin, n_fasce, n_bande), -999.0), rainfall=np.full((n_basin, n_fasce, n_bande), -999.0), 
        sumT=sumT, WE=WE,
        ...
        baseflow=baseflow, baseflow_fasce=np.full((n_basin, n_fasce, n_hours + 1), 0.0),
        baseflow_glac=baseflow_glac, baseflow_glac_fasce=np.full((n_basin, n_fasce, n_hours + 1), 0.0),
        ...
        rhosnow=rhosnow, V_glac_melt=V_glac_melt,
        snow_melt=np.full((n_basin, n_fasce, n_bande), 0.0), lqw=lqw,
        snow_freeze=np.full((n_basin, n_fasce, n_bande), 0.0), snow_freeze_basin=np.full((n_basin, n_hours + 1), 0.0),
        snow_freeze_fasce=np.full((n_basin, n_fasce, n_hours + 1), 0.0),
        WE_basin=np.full((n_basin, n_hours + 1), 0.0), lqw_basin=np.full((n_basin, n_hours + 1), 0.0),
        ...
        beta2=beta2, cmf=cmf,
        nmf=nmf, rmf=rmf,
        c5=c5, c6=c6, eta0=eta0, rhomin=rhomin,
        EI=EI, EImin=1.0,
        sunrise=sunrise, sunset=sunset,
        SCA=np.full( n_hours + 1, 0.0), WE_threshold=WE_THRESHOLD,
        delaytime=delaytime,
    ...
    pd.DataFrame({
        ...
        os.path.join(OUTPUT_FOLDER, 'state_var_SNOW_WE.txt'),
        index=False
        )
    # snow density
    pd.DataFrame({
