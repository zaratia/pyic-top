...
def get_Ra(yy, mm, dd, hh, avg_lat):
    ...
    rad = 15.392 * dr * (
         omega_s * np.sin(phi) * np.sin(delta) + np.cos(phi) *
         np.cos(delta) * np.sin(omega_s)
         )
...
@njit
def PET_Hargreaves(
     avg_lat, n_hours, hour_array, year_array, month_array, day_array, n_basin,
     PET, n_lapse, basin_elev, basin_lapse, t_intercept, t_slope, ecf, dT_month,
     id_lapse_rate, t_idlapse
     ):
    ...
    for time_step in range(1, n_hours + 1, 1):
        ...
        if hour_array[time_step - 1] == 0:
            print('PET_Hargreaves', year_array[time_step - 1], month_array[time_step - 1], day_array[time_step - 1])
            ...
            for ilapse in range(n_lapse):
                if (
                    ...
                    (t_idlapse[time_step - 1] == id_lapse_rate[ilapse])
                        ...
                        t_basin = t_intercept[time_step - 1] + basin_elev[current_basin] * t_slope[time_step - 1]
                        PET[current_basin, time_step] = (
                            ecf * (t_basin + 17.8) * 0.0023 * rad * 
                            ((np.abs(dT_month[current_month, current_basin]))**0.5 ) / 24.
                        )
