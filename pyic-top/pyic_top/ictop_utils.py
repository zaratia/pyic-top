...
def reorder_df_to_seq():
    ...
    df_elevbnds = df_elevbnds.sort_values(by=['seq_order', 'h1']).drop(columns='seq_order')
    
    df_energybnds['seq_order'] = df_energybnds['idba'].map(seq_of_bas)
    df_energybnds = df_energybnds.sort_values(by=['seq_order', 'idbn']).drop(columns='seq_order')
    ...
    df_precipitation = df_precipitation.sort_values(by=['seq_order', 'time']).drop(columns='seq_order')
...
def init_elev_ener_vars(df_elevbnds, df_energybnds, df_EI, n_basin):
    ...
    fasce_area = df_elevbnds.pivot(index=['idba'], columns=['n_bn'],values='area').values
    fasce_elev_avg = df_elevbnds.pivot(index=['idba'], columns=['n_bn'],values='h_avg').values
    ...
    EI = df_EI.pivot(
        index=['idba'], columns=['idmo', 'n_bn', 'n_cl'], values='EI'
        ).values.reshape(n_basin, n_fasce, 12, n_bande).transpose(0,2,1,3)
...
def init_meteo(fprec, ftemp, START_TIME, END_TIME):
    ...
    df_precipitation['idba']=pd.Categorical(
        df_precipitation['idba'], categories=df_precipitation['idba'].unique(), ordered=True
        )
    prec = df_precipitation.pivot(index=['idba'], columns=['time'],values='value').values
...
def init_temper(ftemp, START_TIME, END_TIME):
    ...
    df_temperature = df_temperature[
        (df_temperature['time'] > START_TIME[0:13]) & (df_temperature['time'] <= END_TIME[0:13])
    ]
...
def init_baseflow(fbflow, START_TIME, END_TIME):
    ...
    baseflow = df_baseflow.pivot(index=['idba'], columns=['time'],values='value').values
