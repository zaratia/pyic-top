...
if __name__ == "__main__":
    ...
    for i, row in df_trans_seq.iterrows():
        if row["idmo"] == 5:  # basin, just transfer the flow
            ...
            upstream_nodelink = (
                ...
                .loc[int(df_reservoirs.loc[reservoir_id]["nodein"])]
            )
            downstream_nodelink_turb = (
                df_nodelinks.reset_index()
                .set_index("nodelink_node_u")
                .loc[int(df_reservoirs.loc[reservoir_id]["nodeturb"])]
            )
            ...
            id_nodelinks_seq[nodelink_count] = downstream_nodelink_spill["nodelink_id"]
            nodelink_count += 1
            ...
            upstream_node = (
                df_reaches.reset_index().set_index("idre").loc[reach_id]["idin"]
            )
            upstream_nodelink = (
                df_nodelinks.reset_index()
                .set_index("nodelink_node_d")
            ...
            downstream_node = (
                df_reaches.reset_index().set_index("idre").loc[reach_id]["idout"]
            )
            downstream_nodelink = (
                df_nodelinks.reset_index()
                .set_index("nodelink_node_u")
            ...
            upstream_node1 = (
                df_junctions.reset_index().set_index("idju").loc[junction_id]["idin1"]
            )
            upstream_nodelink1 = (
                df_nodelinks.reset_index()
                .set_index("nodelink_node_d")
            ...
            upstream_node2 = (
                df_junctions.reset_index().set_index("idju").loc[junction_id]["idin2"]
            )
            upstream_nodelink2 = (
                df_nodelinks.reset_index()
                .set_index("nodelink_node_d")
            ...
            downstream_node = (
                df_junctions.reset_index().set_index("idju").loc[junction_id]["idout"]
            )
            downstream_nodelink = (
                df_nodelinks.reset_index()
                .set_index("nodelink_node_u")
    ...
    df_nodelinks = sort_df_by_seq(
        df_nodelinks.reset_index(), id_nodelinks_seq, "nodelink_id"
    )
    df_nodelinks.to_csv(
    ...
    df_reservoirs = sort_df_by_seq(
        df_reservoirs.reset_index(), id_reservoirs_seq, "idrs"
    )
    df_reservoirs.to_csv(
        os.path.join(
            INPUT_FOLDER, TOPOLOGICAL_ELEMENT_FOLDER, "reservoirs_ordered.txt"
        ),
        index=False,
        float_format=FLOAT_FORMAT_SM,
    )
